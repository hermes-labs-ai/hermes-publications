...
|---|---|---|---|---|
| E-017 | Publication feed and discovery assets are frozen in a local repository commit. | `CONFIRMED_LOCAL` | `<local-ops-workspace>/repositories/hermes-publications`; commit `1580909` | Local branch only; not pushed. Supersedes E-012 only by adding commit identity. |
| E-018 | The isolated website candidate serves the exact canonical feed and advertises it in root HTML. | `CONFIRMED_LOCAL` | `<local-ops-workspace>/tmp/hermes-site-atom-20260802`; commit `eb7b71a`; base `ebe8f15` | Local worktree only; no public endpoint claim. |
